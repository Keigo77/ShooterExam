...
namespace Assets.PixelFantasy.Common.Scripts.Utils
{
    /// <summary>
    ...
    public static class TextureHelper
    {
        ...
        public static Color32[] Repaint(Color32[] pixels, int width, Color32 paint, List<Color32> palette)
        {
            ...
            if (colors.Count != 2 && colors.Count != 3 && colors.Count != 4)
            {
                throw new NotSupportedException("Sprite should have 2, 3 and 4 colors only (+black outline).");
            }
            ...
            var replacement = palette.GetRange(palette.IndexOf(paint) - 1, colors.Count).OrderBy(i => ((Color) i).grayscale).ToList();
            var match = new Dictionary<Color32, Color32>();

            for (var i = 0; i < colors.Count; i++)
            {
                match.Add(colors[i], replacement[i]);
            }
